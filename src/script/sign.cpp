...
uint256 TransactionSignatureCreator::GetSigHash() const
{
    CScript scriptCode;
    return SignatureHash(scriptCode, *txTo, nIn, nHashType, amount, SIGVERSION_WITNESS_V0);
}
...
bool ProduceSignature(const BaseSignatureCreator& creator, const CScript& fromPubKey, SignatureData& sigdata)
{
    ...
    else if (solved && whichType == TX_WITNESS_V2_PQKEYHASH)
    {
        ...
        if (!creator.KeyStore().GetPQKey(pqHash, privkey_bytes, pubkey_bytes)) {
            solved = false;
        } else {
            CPQKey pqKey;
            if (!pqKey.SetRaw(privkey_bytes)) {
                ...
                uint256 sighash = creator.GetSigHash();
                std::vector<uint8_t> hash_vec(sighash.begin(), sighash.end());
                std::vector<uint8_t> sig = pqKey.Sign(hash_vec);
                if (sig.empty()) {
                    solved = false;
                } else {
                    sigdata.scriptWitness.stack.clear();
                    sigdata.scriptWitness.stack.push_back(sig);
                    sigdata.scriptWitness.stack.push_back(pubkey_bytes);
                }
            }
        }
        result.clear();
