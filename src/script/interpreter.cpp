...
{
    CScript scriptCode;
    return SignatureHash(scriptCode, *txTo, nIn, SIGHASH_ALL, amount,
...
static bool VerifyWitnessProgram(const CScriptWitness& witness, int witversion, const std::vector<unsigned char>& program, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror)
{
    ...
    } else if (witversion == 2 && program.size() == 20) {
        // PQ witness v2: ML-DSA-44 signature
        // witness stack: [<pq_signature> <pq_pubkey>]
        if (witness.stack.size() != 2) {
            return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
        }
        const std::vector<unsigned char>& pq_pubkey_bytes = witness.stack[1];
        const std::vector<unsigned char>& pq_sig_bytes    = witness.stack[0];
        if (pq_pubkey_bytes.size() != 1312) {
            return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
        }
        uint8_t sha256out[32];
        CSHA256().Write(pq_pubkey_bytes.data(), pq_pubkey_bytes.size()).Finalize(sha256out);
        uint8_t hash160out[20];
        CRIPEMD160().Write(sha256out, 32).Finalize(hash160out);
        if (memcmp(hash160out, program.data(), 20) != 0) {
            return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
        }
        uint256 sighash = checker.GetSigHash();
        std::vector<uint8_t> hash_vec(sighash.begin(), sighash.end());
        CPQPubKey pqpub;
        pqpub.SetRaw(pq_pubkey_bytes);
        if (!pqpub.Verify(hash_vec, pq_sig_bytes)) {
            return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
        }
        return set_success(serror);
