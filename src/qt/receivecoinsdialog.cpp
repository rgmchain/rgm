...
void ReceiveCoinsDialog::on_receiveButton_clicked()
{
    ...
    OutputType addrType = selectedAddressType();

    // Guard: SegWit address types require SegWit to be active on the network.
    // Show a friendly error rather than generating an address that can't be spent yet.
    if (addrType != OutputType::LEGACY) {
        LOCK(cs_main);

        const int currentHeight = chainActive.Height();
        const int segwitHeight = Params().GetConsensus(currentHeight).nSegwitHeight;
            ...
            QMessageBox::warning(this, tr("SegWit not active"),
                tr("Bech32 (native SegWit) addresses are not yet active on the network.\n\n"
                   ...
                   "Please select \"Legacy\" address type.")
                    .arg(QString::number(currentHeight))
                    .arg(QString::number(segwitHeight)));
            return;
        }
    }
    ...
    if(ui->reuseAddress->isChecked())
    {
        ...
        /* Generate new receiving address of the selected type */
        address = model->getAddressTableModel()->addRow(
            AddressTableModel::Receive, label, "", addrType);
    }
