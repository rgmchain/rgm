...
const QString BITCOIN_IPC_PREFIX("rgm:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
        {
            ...
            {
                // Detect network from address prefix without constructing CBitcoinAddress.
                // We temporarily select each network and try DecodeDestination.
                const std::string addrStr = r.address.toStdString();

                SelectParams(CBaseChainParams::MAIN);
                if (IsValidDestination(DecodeDestination(addrStr))) {
                    // already selected main, nothing to do
                } else {
                    SelectParams(CBaseChainParams::TESTNET);
                    if (!IsValidDestination(DecodeDestination(addrStr))) {
                        // Not valid on testnet either — leave testnet selected;
                        // the user will get a "wrong network" error later.
                    }
                }
            }
        }
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(ipcServerName)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start rgm: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        {
            CTxDestination dest = DecodeDestination(recipient.address.toStdString());
            ...
            Q_EMIT message(tr("URI handling"),
                tr("URI cannot be parsed! This can be caused by an invalid RGM address or malformed URI parameters."),
