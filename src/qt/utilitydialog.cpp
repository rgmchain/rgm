    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    if (about)
    {
        ...
        QString header = tr("Usage:") + "\n" +
            "  rgm-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", RgmGUI::DEFAULT_UIPLATFORM));
        }
...
void PaperWalletDialog::on_printButton_clicked()
{
    ...
    while (true) {
        ...
        // Ask for an amount to send to each paper wallet. It might be better to try to use the RgmAmountField, but this works fine.
    ...
    if (txFee > 0) {
        ...
        questionString.append(RgmUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee));
    ...
    Q_FOREACH (RgmUnits::Unit u, RgmUnits::availableUnits()) {
        if (u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(RgmUnits::formatWithUnit(u, totalAmount));
    }

    questionString.append(tr("Total Amount %1 (= %2)")
                              .arg(RgmUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount))
...
QWidget *ShutdownWindow::showShutdownWindow(RgmGUI *window)
{
    if (!window)
        return nullptr;
