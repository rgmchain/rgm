...
UniValue generateBlocks(std::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript, int nMineAuxPow)
{
    ...
    while (nHeight < nHeightEnd)
    {
        ...
        const Consensus::Params& nextConsensus = Params().GetConsensus(nHeight + 1);

        if (!nMineAuxPow && !nextConsensus.fAllowLegacyBlocks) {
            throw JSONRPCError(RPC_INVALID_PARAMETER,
                "Legacy mining is not allowed after AuxPoW activation. Use getauxblock/submitauxblock.");
        }

        if (nMineAuxPow) {
            throw JSONRPCError(RPC_INVALID_PARAMETER,
                "AuxPoW mining through generate/generatetoaddress is not supported. Use getauxblock/submitauxblock.");
        }
        ...
        // RGM: generate/generatetoaddress поддерживают только legacy mining.
        // После AuxPoW activation нужно использовать getauxblock/submitauxblock.
        while (nMaxTries > 0 &&
               pblock->nNonce < nInnerLoopCount &&
               !CheckProofOfWork(pblock->GetPoWHash(), pblock->nBits, Params().GetConsensus(nHeight + 1))) {
            ++pblock->nNonce;
            --nMaxTries;
        }
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. nblocks      (numeric, required) How many blocks are generated immediately.\n"
            "2. address      (string, required) The address to send the newly generated RGM to.\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "RGM is downloading blocks...");
