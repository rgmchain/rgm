...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    // RGM: If the coinbase contains a SegWit witness commitment (BIP141 / OP_RETURN 0xaa21a9ed...),
    // expose it explicitly so explorers can display it without parsing raw scripts.
    if (!block.vtx.empty()) {
        const CTransaction& coinbaseTx = *block.vtx[0];
        for (const CTxOut& out : coinbaseTx.vout) {
            // Witness commitment scriptPubKey: OP_RETURN (0x6a) <36 bytes> 0xaa21a9ed <32-byte hash>
            if (out.scriptPubKey.size() >= 38 &&
            ...
            {
                result.pushKV("witness_commitment",
                    HexStr(out.scriptPubKey.begin() + 6, out.scriptPubKey.end()));
                break;
            }
        }
    }
...
UniValue getblockheader(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "  \"witness_commitment\" : \"hex\",  (string, optional) SegWit witness commitment from coinbase (if present)\n"
            "}\n"
            "\nResult (for verbose=false):\n"
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of RGM addresses\n"
