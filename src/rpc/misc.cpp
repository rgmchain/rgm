...
public:
    ...
    // Native P2WPKH — bech32 rgm1q... (42 chars)
    ...
    // Native P2WSH — bech32, longer than P2WPKH
    ...
    // Unknown future witness version
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nReturn information about the given RGM address.\n"
            "Supports legacy base58 (R...) and native bech32 (rgm1.../rrgm1...) addresses.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The RGM address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not.\n"
            "  \"address\" : \"address\",         (string) The RGM address validated\n"
            "  \"scriptPubKey\" : \"hex\",         (string) The hex encoded scriptPubKey\n"
            "  \"ismine\" : true|false,          (boolean) If the address is yours or not\n"
            "  \"iswatchonly\" : true|false,     (boolean) If the address is watchonly\n"
            "  \"isscript\" : true|false,        (boolean) If the key is a script\n"
            "  \"iswitness\" : true|false,       (boolean) If this is a native witness address\n"
            "  \"witness_version\" : n,          (numeric, optional) Witness version (0 for P2WPKH/P2WSH)\n"
            "  \"witness_program\" : \"hex\",    (string, optional) The witness program hash\n"
            "  \"pubkey\" : \"publickeyhex\",    (string, optional) The hex value of the raw public key\n"
            "  \"iscompressed\" : true|false,    (boolean, optional) If the address is compressed\n"
            "  \"account\" : \"account\",        (string) DEPRECATED. The account associated with the address\n"
            "  \"timestamp\" : timestamp,        (number, optional) The creation time of the key\n"
            "  \"hdkeypath\" : \"keypath\",      (string, optional) The HD keypath if the key is HD\n"
            "  \"hdmasterkeyid\" : \"<hash160>\" (string, optional) The Hash160 of the HD master pubkey\n"
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("validateaddress", "\"RXyzAbc...\"")
            + HelpExampleCli("validateaddress", "\"rrgm1q...\"")
            + HelpExampleRpc("validateaddress", "\"rgm1q...\"")
        );
    ...
    // Decode supports both base58 (R...) and bech32 (rgm1.../rrgm1...)
    ...
    if (isValid)
    {
        ...
        ret.pushKVs(detail);
        if (pwalletMain && dest.type() != typeid(WitnessUnknown) && pwalletMain->mapAddressBook.count(dest))
            ret.pushKV("account", pwalletMain->mapAddressBook[dest].name);

        if (pwalletMain) {
            const auto& meta = pwalletMain->mapKeyMetadata;

            // Find key metadata: check keyID for legacy P2PKH and native P2WPKH
            CKeyID keyID;
            if (dest.type() == typeid(CKeyID)) {
                keyID = boost::get<CKeyID>(dest);
            } else if (dest.type() == typeid(WitnessV0KeyHash)) {
                keyID = CKeyID(boost::get<WitnessV0KeyHash>(dest));
            }

            auto it = (!keyID.IsNull()) ? meta.find(keyID) : meta.end();

            // For P2SH fall back to scriptPubKey-based lookup
            if (it == meta.end()) {
...
#ifdef ENABLE_WALLET
        // Case 1: адрес (legacy base58 или native bech32) — достаём pubkey из wallet
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are RGM addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) RGM address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The RGM address to use for the signature.\n"
    ...
    // Поддерживаем legacy R... (CKeyID) и native bech32 rgm1... (WitnessV0KeyHash)
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw runtime_error(
            ...
            "rgm-cli and the GUI. There is no server-side difference."
        );
