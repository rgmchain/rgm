...
#if defined(NDEBUG)
# error "RGM cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "RGM Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetRGMMinRelayFee(tx, nSize, !fLimitFree))
        {
...
static bool ReadBlockOrHeader(T& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams, bool fCheckPOW)
{
    ...
    // Check the header, cheack AuxPoW if legacy  no 
    // RGM: универсальная проверка
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // RGM: Switch maturity at depth 145,000
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // RGM: BIP30 has been active since inception
    ...
    // RGM: BIP16 has been enabled since inception
    ...
    CAmount nBlockSubsidy = GetRGMBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
    // RGM: Burn механика - 2% комиссий сжигается, майнер получает только 98%
    CAmount nBurnAmount = nFees / 50;        // 2% burn
    CAmount nMinerFees  = nFees - nBurnAmount; // 98% майнеру
    CAmount blockReward = nMinerFees + nBlockSubsidy;
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    // RGM: универсальная PoW проверка без выбора этапа — высоты тут нет.
    // Строгая chainId проверка (fStrictChainId) происходит в ContextualCheckBlockHeader.
    if (fCheckPOW && !CheckAuxPowProofOfWork(block, consensusParams))
...
{
    // RGM: SegWit включается автоматически на высоте nSegwitHeight
    // Зашито в chainparams, не требует хард форка
    if (pindexPrev == nullptr)
        return false;
    return (pindexPrev->nHeight + 1) >= params.nSegwitHeight;
}
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // RGM: Запрещаем legacy блоки после nAuxpowHeight
    if (!consensusParams.fAllowLegacyBlocks && block.IsLegacy())
        return state.DoS(100, error("%s : legacy block not allowed at height %d (auxpow started at %d)",
                                    __func__, nHeight, consensusParams.nAuxpowHeight),
                         REJECT_INVALID, "late-legacy-block");

    // RGM: Запрещаем AuxPoW блоки до nAuxpowHeight
    if (consensusParams.fAllowLegacyBlocks && block.IsAuxpow())
        return state.DoS(100, error("%s : auxpow block not allowed at height %d (auxpow starts at %d)",
                                    __func__, nHeight, consensusParams.nAuxpowHeight),
                         REJECT_INVALID, "early-auxpow-block");

    // RGM: проверяем только сложность (nBits), не PoW хеш.
    // Полная PoW проверка — в CheckBlockHeader и AcceptBlock.
    // Здесь CheckAuxPowProofOfWork НЕ вызываем — шаблон блока проверяется
    // до подбора nonce и упадёт в CreateNewBlock/generatetoaddress.
    ...
    // RGM: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // RGM: We probably want to disable this
    int nLockTimeFlags = 0;
    ...
    // RGM: SegWit по высоте nSegwitHeight, не через BIP9 (который выключен)
            ...
            try {
                ...
                while (!queue.empty()) {
                    ...
                    while (range.first != range.second) {
                        ...
                        // RGM: используем высоту родителя из mapBlockIndex
                        auto miRecursive = mapBlockIndex.find(head);
                        int nRecursiveHeight = (miRecursive != mapBlockIndex.end() && miRecursive->second)
                            ? miRecursive->second->nHeight + 1
                            : 0;
                        if (ReadBlockFromDisk(*pblockrecursive, it->second, chainparams.GetConsensus(nRecursiveHeight)))
                        {
