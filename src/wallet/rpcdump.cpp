...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (IsValidDestination(dest)) {
        ...
        // Импортируем как watch-only через script
        CScript script = GetScriptForDestination(dest);
        ImportScript(script, strLabel, false);
        // Всегда устанавливаем label для валидного destination
        pwalletMain->SetAddressBook(dest, strLabel, "receive");
    } else if (IsHex(request.params[0].get_str())) {
        std::vector<unsigned char> data(ParseHex(request.params[0].get_str()));
        ImportScript(CScript(data.begin(), data.end()), strLabel, fP2SH);
    } else {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid RGM address or script");
    }
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The RGM address for the private key\n"
            "\nResult:\n"
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by RGM %s\n", CLIENT_BUILD);
...
UniValue ProcessImport(const UniValue& data, const int64_t timestamp)
{
    try {
        ...
        // Parse the output.
        CScript script;
        CTxDestination destImport;

        if (!isScript) {
            destImport = DecodeDestination(output);
            if (!IsValidDestination(destImport)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
            }
            script = GetScriptForDestination(destImport);
        } else {
            if (!IsHex(output)) {
        ...
        if (isP2SH) {
            ...
            CScript redeemDestination = GetScriptForDestination(redeemScriptID);
            ...
            // add to address book or update label
            if (IsValidDestination(destImport)) {
                pwalletMain->SetAddressBook(destImport, label, "receive");
            }
            ...
            if (pubKeys.size() && keys.size() == 0) {
                ...
                CBitcoinAddress pubKeyAddress = CBitcoinAddress(pubKeyID);

                // Consistency check: destImport должен соответствовать этому pubkey.
                // Поддерживаем CKeyID (legacy), WitnessV0KeyHash (bech32), CScriptID (p2sh-segwit).
                if (!isScript) {
                    CKeyID destKeyID;
                    if (const CKeyID* id = boost::get<CKeyID>(&destImport))
                        destKeyID = *id;
                    else if (const WitnessV0KeyHash* id = boost::get<WitnessV0KeyHash>(&destImport))
                        destKeyID = CKeyID(*id);
                    else
                        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Address does not refer to a key");
                    if (destKeyID != pubKeyID)
                        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Consistency check failed");
                }

                // Consistency check для script-пути.
                if (isScript) {
                    CTxDestination destination;
                    if (ExtractDestination(script, destination)) {
                        CKeyID scriptKeyID;
                        if (const CKeyID* id = boost::get<CKeyID>(&destination))
                            scriptKeyID = *id;
                        else if (const WitnessV0KeyHash* id = boost::get<WitnessV0KeyHash>(&destination))
                            scriptKeyID = CKeyID(*id);
                        else
                            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Address does not refer to a key");
                        if (scriptKeyID != pubKeyID)
                            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Consistency check failed");
                    }
                }

                CScript pubKeyScript = GetScriptForDestination(destImport);
                ...
                // add to address book or update label — используем destImport (может быть bech32)
                if (IsValidDestination(destImport)) {
                    pwalletMain->SetAddressBook(destImport, label, "receive");
                }
            ...
            if (keys.size()) {
                ...
                // Consistency check: destImport должен соответствовать этому ключу.
                if (!isScript) {
                    CKeyID destKeyID;
                    if (const CKeyID* id = boost::get<CKeyID>(&destImport))
                        destKeyID = *id;
                ...
                CKeyID vchAddress = pubKey.GetID();
                pwalletMain->MarkDirty();
                if (!isScript && IsValidDestination(destImport))
                    pwalletMain->SetAddressBook(destImport, label, "receive");
                else
                    pwalletMain->SetAddressBook(vchAddress, label, "receive");
