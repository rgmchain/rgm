...
#ifndef RGM_WALLET_WALLET_H
#define RGM_WALLET_WALLET_H
...
/*
 * RGM: Scaled to 1/10th of the recommended transaction fee to make RBF
...
/*
 * RGM: Creating change outputs at exactly the dustlimit is counter-
...
public:
    ...
    //! Get a destination of the requested output type for a given public key.
    //! For P2SH_SEGWIT, also stores the redeemScript via AddCScript().
    CTxDestination GetDestinationForKey(const CPubKey& key, OutputType type);
    //! Adds a key to the store, and saves it to disk.
    bool AddKeyPubKey(const CKey& key, const CPubKey &pubkey) override;
    //! Adds a PQ (ML-DSA-44) keypair to the wallet and saves to disk
    bool AddPQKey(const CPQKey& key, const CPQPubKey& pubkey);
    bool GetPQKey(const uint160& hash160, std::vector<uint8_t>& privkeyOut, std::vector<uint8_t>& pubkeyOut) const override;
    ...
    /**
     * RGM: Get a fee targeting a specific transaction speed.
     */
    CAmount GetRGMPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed);
    /**
     * RGM: Get a fee targeting a specific transaction speed.
     */
    static CAmount GetRGMPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nSpeed, CAmount targetFee);
    /**
    ...
    /**
     * Return the minimum required fee taking into account the
     * floating relay fee and user set minimum transaction fee, but not the RGM dust fee.
     */
