...
// Copyright (c) 2022-2023 The RGM Core developers
...
UniValue getnewaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 2)
        throw runtime_error(
            ...
            "\nReturns a new RGM address for receiving payments.\n"
            "If 'account' is specified (DEPRECATED), it is added to the address book\n"
            "so payments received with the address will be credited to 'account'.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, optional) DEPRECATED. The account name for the address.\n"
            "                     Use \"\" for the default account.\n"
            "2. \"address_type\"  (string, optional) The address type to use:\n"
            "                     \"legacy\"      - base58 R... address (default)\n"
            "                     \"p2sh-segwit\" - P2SH-wrapped SegWit\n"
            "                     \"bech32\"      - native bech32 rgm1.../rrgm1...\n"
            "                     \"pq\"           - ML-DSA-44 post-quantum address\n"
            "\nResult:\n"
            "\"address\"    (string) The new RGM address\n"
            "\nExamples:\n"
    ...
    // Parse account (deprecated but kept for compatibility)
    string strAccount;
    if (request.params.size() > 0)
        strAccount = AccountFromValue(request.params[0]);

    // Parse optional address_type argument
    ...
    // Handle PQ address type separately
    if (output_type == OutputType::PQ) {
        CPQPubKey pqPubKey;
        CPQKey pqKey;
        if (!pqKey.MakeNewKey(pqPubKey))
            throw JSONRPCError(RPC_WALLET_ERROR, "Error: PQ key generation failed");
        if (!pwalletMain->AddPQKey(pqKey, pqPubKey))
            throw JSONRPCError(RPC_WALLET_ERROR, "Error: Failed to store PQ key");
        std::vector<uint8_t> hash160 = pqPubKey.GetHash160();
        WitnessUnknown pqDest;
        pqDest.version = 2;
        pqDest.length = 20;
        std::copy(hash160.begin(), hash160.end(), pqDest.program);
        CTxDestination dest = pqDest;
        pwalletMain->SetAddressBook(dest, strAccount, "receive");
        return EncodeDestination(dest);
    }
    // Get destination of the requested type.
    // For P2SH_SEGWIT, CWallet::GetDestinationForKey also calls AddCScript().
    CTxDestination dest = pwalletMain->GetDestinationForKey(newKey, output_type);
...
UniValue getaccountaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current RGM address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"address\"          (string) The account RGM address\n"
            "\nExamples:\n"
...
UniValue getrawchangeaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1)
        throw runtime_error(
            "getrawchangeaddress\n"
            "\nReturns a new RGM address, for receiving change.\n"
...
UniValue setaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The RGM address to be associated with an account.\n"
    ...
    if (!IsValidDestination(dest))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid RGM address");
    ...
    if (IsMine(*pwalletMain, dest))
    {
        ...
        if (pwalletMain->mapAddressBook.count(dest))
        {
            ...
            // Сравниваем CTxDestination напрямую — работает для любого типа адреса
            CBitcoinAddress oldAddr = GetAccountAddress(strOldAccount);
            CTxDestination oldDest = oldAddr.Get();
            if (dest == oldDest)
...
UniValue getaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The RGM address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "  \"address\"         (string) an RGM address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"            (string, required) The RGM address to send to.\n"
...
UniValue listaddressgroupings(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"address\",            (string) The RGM address\n"
...
UniValue signmessage(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2)
        throw runtime_error(
            ...
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            "1. \"address\"         (string, required) The RGM address to use for the private key.\n"
    ...
    // Поддерживаем оба формата: legacy R... (CKeyID) и native bech32 rgm1... (WitnessV0KeyHash)
    // Оба имеют underlying CKeyID — подпись работает одинаково
    ...
    if (const CKeyID* id = boost::get<CKeyID>(&dest)) {
        keyID = *id;
    } else if (const WitnessV0KeyHash* id = boost::get<WitnessV0KeyHash>(&dest)) {
        keyID = CKeyID(*id);
    } else {
        throw JSONRPCError(RPC_TYPE_ERROR, "Address does not refer to a key (P2PKH or P2WPKH only)");
    }
...
UniValue getreceivedbyaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The RGM address for transactions.\n"
...
UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 6)
        throw runtime_error(
            ...
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to an RGM address."
            + HelpRequiringPassphrase() + "\n"
            "\nArguments:\n"
            ...
            "                       the spend.\n"
            "2. \"toaddress\"         (string, required) The RGM address to send funds to.\n"
...
UniValue sendmany(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The RGM address is the key, the numeric amount (can be string) in " + CURRENCY_UNIT + " is the value\n"
            "      ,...\n"
            "    }\n"
    ...
    BOOST_FOREACH(const string& name_, keys)
    {
        ...
        if (!IsValidDestination(dest))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid RGM address: ")+name_);
...
UniValue addmultisigaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
    {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
            ...
            "Each key is a RGM address or hex-encoded public key.\n"
            "If 'account' is specified (DEPRECATED), assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
            "2. \"keys\"         (string, required) A json array of RGM addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) RGM address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) DEPRECATED. An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"address\"         (string) A RGM address associated with the keys.\n"
...
public:
    ...
    // Native P2WPKH — уже является witness, addwitnessaddress не нужен
    bool operator()(const WitnessV0KeyHash&)    { return false; }
    // Native P2WSH — уже является witness
    bool operator()(const WitnessV0ScriptHash&) { return false; }
    // Неизвестная witness версия — не поддерживаем
    bool operator()(const WitnessUnknown&)      { return false; }
};
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"address\",    (string) The RGM address of the transaction. Not present for \n"
    ...
    if (request.fHelp || request.params.size() > 2)
        throw runtime_error(
           ...
           "[                     (json array of string)\n"
            ...
            "         \"address\" : \"address\",          (string) The RGM address involved in the transaction\n"
...
UniValue listsinceblock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp)
        throw runtime_error(
            ...
            "    \"address\":\"address\",    (string) The RGM address of the transaction. Not present for move transactions (category = move).\n"
...
UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    StartShutdown();
    return "wallet encrypted; RGM server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
...
UniValue listunspent(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 5)
        throw runtime_error(
            ...
            "3. \"addresses\"      (string) A json array of RGM addresses to filter\n"
            "    [\n"
            "      \"address\"     (string) RGM address\n"
            "      ,...\n"
            "    ]\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",    (string) the RGM address\n"
    ...
    if (request.params.size() > 2 && !request.params[2].isNull()) {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid RGM address: ")+input.get_str());
            if (setAddress.count(dest))
...
UniValue fundrawtransaction(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw runtime_error(
                            ...
                            "   {\n"
                            "     \"changeAddress\"          (string, optional, default pool address) The RGM address to receive the change\n"
    ...
    if (request.params.size() > 1) {
      if (request.params[1].type() == UniValue::VBOOL) {
        ...
        if (options.exists("changeAddress")) {
            ...
            if (!IsValidDestination(dest))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "changeAddress must be a valid RGM address");
            changeAddress = dest;
        }
