...
//! Default output type for new addresses (legacy = R... base58)
...
/**
 * RGM: Effective dust limit for the wallet
...
CTxDestination CWallet::GetDestinationForKey(const CPubKey& key, OutputType type)
{
    ...
    case OutputType::LEGACY:
        // Standard P2PKH: legacy R... address
        return key.GetID();

    case OutputType::P2SH_SEGWIT: {
        // P2SH-wrapped P2WPKH.
        // Build witness script (OP_0 <20-byte-hash>), store it so wallet
        // can redeem later, then return P2SH destination.
        WitnessV0KeyHash witnessKeyHash(key.GetID());
        CScript witnessScript = GetScriptForDestination(witnessKeyHash);
        AddCScript(witnessScript);   // MUST be called here — not in outputtype.cpp
        return CScriptID(witnessScript);
    }

    case OutputType::BECH32:
        // Native P2WPKH — bech32 rgm1q... / rrgm1q...
...
bool CWallet::GetPQKey(const uint160& hash160, std::vector<uint8_t>& privkeyOut, std::vector<uint8_t>& pubkeyOut) const
{
    return CBasicKeyStore::GetPQKey(hash160, privkeyOut, pubkeyOut);
}

bool CWallet::AddPQKey(const CPQKey& key, const CPQPubKey& pubkey)
{
    AssertLockHeld(cs_wallet);
    if (!pubkey.IsValid() || !key.IsValid())
        return false;
    // Add to in-memory keystore immediately
    std::vector<uint8_t> hash160 = pubkey.GetHash160();
    uint160 hash160u;
    memcpy(hash160u.begin(), hash160.data(), 20);
    AddPQKeyToStore(hash160u, key.GetRaw(), pubkey.GetRaw());
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WritePQKey(
        pubkey.GetRaw(),
        key.GetRaw()
    );
}
...
// RGM: MIN_CHANGE as a function of discardThreshold and minTxFee(1000)
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                for (const auto& recipient : vecSend)
                {
                    ...
                    /*
                     * RGM: check all outputs against the discard threshold
                ...
                if (nPriority == MINIMUM) {
                    ...
                    nFeeNeeded = GetRGMPriorityFee(txNew, nBytes, nPriority);
                }
                ...
                if (nChangePosInOut != -1 && nSubtractFeeFromAmount == 0) {
                    ...
                    /* RGM: this has been changed from a static MIN_FINAL_CHANGE that
...
{
    // RGM: Add an increased fee for each output that is lower than the discard threshold
    return std::max(minTxFee.GetFee(nTxBytes) + GetRGMDustFee(tx.vout, discardThreshold), ::minRelayTxFeeRate.GetFee(nTxBytes));
}
...
CAmount CWallet::GetMinimumFee(const CMutableTransaction& tx, unsigned int nTxBytes, unsigned int nConfirmTarget, const CTxMemPool& pool, CAmount targetFee)
{
    ...
    if (nFeeNeeded == 0) {
        ...
        // RGM: Drop the smart fee estimate, use GetRequiredFee
        nFeeNeeded = GetRequiredFee(tx, nTxBytes);
    }
    // prevent user from paying a fee below minRelayTxFee or minTxFee
    // RGM: as we're adapting minTxFee to never be higher than
...
CAmount CWallet::GetRGMPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nPriority)
{
    // payTxFee is the user-set global for desired feerate
    return GetRGMPriorityFee(tx, nTxBytes, nPriority, payTxFee.GetFee(nTxBytes));
}
CAmount CWallet::GetRGMPriorityFee(const CMutableTransaction& tx, unsigned int nTxBytes, FeeRatePreset nPriority, CAmount targetFee)
{
    CAmount nFeeNeeded = targetFee;
    // User didn't set: use -txconfirmtarget to estimate...
    if (nFeeNeeded == 0) {
...
public:
    ...
    // Native P2WPKH: извлекаем keyID из hash160 witness программы
    ...
    // P2WSH: для полной поддержки нужен witnessScript по SHA256,
    // в текущей реализации пропускаем
    void operator()(const WitnessV0ScriptHash &) {}

    // Неизвестная witness версия — нет известного keyID
    void operator()(const WitnessUnknown &) {}
...
std::string CWallet::GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-addresstype=<type>", _("What type of addresses to use by default (\"legacy\", \"p2sh-segwit\", or \"bech32\"). Default: \"legacy\"."));
    strUsage += HelpMessageOpt("-changetype=<type>", _("What type of change to use (\"legacy\", \"p2sh-segwit\", or \"bech32\"). Default: \"legacy\"."));
...
bool CWallet::ParameterInteraction()
{
    ...
    // Тип адреса по умолчанию для getnewaddress и сдачи
    if (IsArgSet("-addresstype")) {
        if (!ParseOutputType(GetArg("-addresstype", ""), g_address_type)) {
            return InitError(strprintf(_("Unknown address type '%s'"), GetArg("-addresstype", "")));
        }
    }
    if (IsArgSet("-changetype")) {
        if (!ParseOutputType(GetArg("-changetype", ""), g_change_type)) {
