...
bool CWalletDB::WritePQKey(const std::vector<uint8_t>& pubkey,
                           const std::vector<uint8_t>& privkey)
{
    nWalletDBUpdateCounter++;
    return Write(std::make_pair(std::string("pqkey"), pubkey), privkey, false);
}
    ...
    try {
        ...
        if (strType == "name")
        {
            ...
            if (!IsValidDestination(dest)) {
                LogPrintf("walletdb: skipping invalid address book name entry '%s'\n", strAddress);
                return true;
            }
        ...
        else if (strType == "purpose")
        {
            ...
            CTxDestination dest = DecodeDestination(strAddress);
            if (!IsValidDestination(dest)) {
                LogPrintf("walletdb: skipping invalid address book purpose entry '%s'\n", strAddress);
                return true;
            }
        ...
        else if (strType == "destdata")
        {
            ...
            CTxDestination dest = DecodeDestination(strAddress);
            if (!IsValidDestination(dest)) {
                LogPrintf("walletdb: skipping invalid destdata entry '%s'\n", strAddress);
                return true;
            }
        ...
        {
            // PQ key: key=pubkey_bytes, value=privkey_bytes
            std::vector<uint8_t> pubkey_bytes;
            ssKey >> pubkey_bytes;
            std::vector<uint8_t> privkey_bytes;
            ssValue >> privkey_bytes;
            CPQKey pqKey;
            CPQPubKey pqPubKey;
            pqPubKey.SetRaw(pubkey_bytes);
            if (!pqKey.SetRaw(privkey_bytes)) {
                strErr = "Error reading wallet database: invalid PQ key";
                return false;
            }
            std::vector<uint8_t> hash160 = pqPubKey.GetHash160();
            uint160 hash160u;
            memcpy(hash160u.begin(), hash160.data(), 20);
            pwallet->AddPQKeyToStore(hash160u, privkey_bytes, pubkey_bytes);
            wss.nKeys++;
        }
    } catch (...)
    {
